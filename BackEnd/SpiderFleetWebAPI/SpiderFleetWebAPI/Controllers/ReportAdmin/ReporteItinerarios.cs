...
{
    public class ReporteItinerarios
    {
        private UseFul use = new UseFul();
        private VariableConfiguration configuration = new VariableConfiguration();

        public MemoryStream Reporte(int type, bool band, string tempOutput, string path, 
            ...
            try
            {
                ...
                foreach (var device in devices)
                {
                    ...
                    using (ExcelPackage excelPackage = new ExcelPackage(memoryStream))
                    {
                        ...
                        information = (new ReportItinerarioDao()).Read(device, nowIni, nowFin, maxVelocidad);
